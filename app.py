...
.k-level.active {
    ...
    box-shadow:0 0 0 1px rgba(99,102,241,.25),0 12px 28px rgba(0,0,0,.18) !important;
}
.k-level.locked { opacity:.62; }
.k-level h4 {
...
if menu == "🏠 Ana Sayfa":
    ...
    level_topics = {
        "A1": ["Temel kelimeler", "Günlük hayat", "Basit cümleler", "Dinleme & konuşma"],
        "A2": ["Geçmiş zaman", "Geniş zaman", "Soru cümleleri", "Kelime dağarcığı"],
        "B1": ["Fikir belirtme", "Deneyimler", "Gelecek planları", "Akıcı konuşma"],
        "B2": ["Karma grammar", "Koşul cümleleri", "Bağlaçlar", "İleri konuşma"],
    }
    ...
    for i, cefr in enumerate(level_order):
        with map_cols[i]:
            ...
            topic_lines = "".join(f"<li>{topic}</li>" for topic in level_topics[cefr])
            st.markdown(f"""
            <div class="{box}">
