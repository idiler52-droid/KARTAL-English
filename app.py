...
.k-panel h3 { margin:0 0 7px; color:#fff !important; }
.k-panel p { margin:0; color:#cbd5e1 !important; }
.k-big {
    font-size:2.2rem;
    font-weight:800;
    color:#fff !important;
...
.k-level.active {
    ...
    box-shadow:0 0 0 1px rgba(129,140,248,.18),0 10px 25px rgba(0,0,0,.16);
}
.k-level.locked { opacity:.52; }
.k-level .badge {
...
@media(max-width:850px) {
    .k-dashboard { grid-template-columns:1fr; }
    .k-map { grid-template-columns:repeat(2,1fr); }
}
@media(max-width:520px) {
    .k-map { grid-template-columns:1fr; }
}
</style>
""", unsafe_allow_html=True)

# ===== K.A.R.T.A.L. MODERN UI =====
st.markdown("""
<style>
...
.kartal-hero {
    padding: 26px 30px;
    border-radius: 26px;
    background: linear-gradient(135deg, #111827 0%, #273449 55%, #374151 100%);
    color: white;
...
/* Streamlit tema bağımsızlığı: ana içerik açık zeminde koyu metin */
...
[data-testid="stAppViewContainer"] section.main textarea {
    color: #1f2937 !important;
}

[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] h1,
[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] h2,
[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] h3,
[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] h4,
[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] p,
[data-testid="stAppViewContainer"] section.main [data-testid="stMarkdownContainer"] li,
...
/* Hero kartı kendi beyaz metnini korusun */
.kartal-hero,
.kartal-hero * {
    color: white !important;
}

/* Renkli bilgi kartları */
.kartal-pill,
.kartal-pill * {
...
<div class="kartal-hero">
  <div class="eyebrow">English Learning • Smart Coach</div>
  <h1>🦅 K.A.R.T.A.L.</h1>
  <p>Konuş, öğren, geliş. Seviyene göre kişiselleştirilmiş İngilizce pratiği.</p>
</div>
""", unsafe_allow_html=True)
...
# Grammar Coach state
if "grammar_weakness" not in st.session_state:
    st.session_state.grammar_weakness = {}
if "grammar_lesson" not in st.session_state:
    st.session_state.grammar_lesson = None
if "grammar_quiz" not in st.session_state:
    st.session_state.grammar_quiz = None
if "grammar_quiz_index" not in st.session_state:
    st.session_state.grammar_quiz_index = 0
if "grammar_quiz_score" not in st.session_state:
    st.session_state.grammar_quiz_score = 0
if "grammar_quiz_feedback" not in st.session_state:
    st.session_state.grammar_quiz_feedback = None
...
def record_learning_result(topic, correct, xp=0, mode="question"):
    ...
    stats["xp"] += int(xp)
    if mode == "question":
        record_grammar_result(topic, correct)
...
# ============================================================
# K.A.R.T.A.L. GRAMMAR COACH
# ============================================================
GRAMMAR_TOPICS = {
    "Present Simple": {
        "tr": "Geniş zaman / günlük alışkanlıklar",
        "explanation": "I/you/we/they ile fiilin yalın hali; he/she/it ile genellikle -s kullanılır.",
        "examples": ["I work every day.", "She works every day.", "Do you like music?"]
    },
    "Present Continuous": {
        "tr": "Şimdiki zaman",
        "explanation": "Şu anda devam eden işler için am/is/are + fiil-ing kullanılır.",
        "examples": ["I am studying now.", "She is watching TV.", "They are working."]
    },
    "Past Simple": {
        "tr": "Geçmiş zaman",
        "explanation": "Geçmişte tamamlanmış olaylar için kullanılır. Düzenli fiillerde genellikle -ed, düzensiz fiillerde özel ikinci hal kullanılır.",
        "examples": ["I watched a movie yesterday.", "She went home.", "Did you eat breakfast?"]
    },
    "Future": {
        "tr": "Gelecek zaman",
        "explanation": "Gelecek planı veya tahminlerde will ya da be going to kullanılabilir.",
        "examples": ["I will call you tomorrow.", "I am going to study tonight."]
    },
    "Present Perfect": {
        "tr": "Present Perfect",
        "explanation": "Geçmişte olmuş ama zamanı belirtilmeyen veya sonucu şimdiyle bağlantılı deneyimlerde have/has + V3 kullanılır.",
        "examples": ["I have visited London.", "She has never tried sushi."]
    },
    "Questions": {
        "tr": "Soru cümleleri",
        "explanation": "Do/does/did veya be yardımcı fiilleri soru yapısında doğru yerde kullanılmalıdır.",
        "examples": ["Do you like music?", "Does he work here?", "Did you go out?"]
    },
    "Prepositions": {
        "tr": "Edatlar: in / on / at",
        "explanation": "Zaman ve yer ifadelerinde doğru edatı seçmek gerekir: at 7, on Monday, in July.",
        "examples": ["at 7 o'clock", "on Monday", "in July"]
    },
    "Articles": {
        "tr": "a / an / the",
        "explanation": "Tekil sayılabilir isimlerde genel anlamda a/an, belirli bir şeyden bahsederken the kullanılır.",
        "examples": ["I saw a dog.", "She ate an apple.", "The dog was friendly."]
    },
    "Modal Verbs": {
        "tr": "can / should / must",
        "explanation": "Modal fiillerden sonra fiilin yalın hali gelir: can go, should study, must work.",
...
GRAMMAR_QUIZZES = {
    "Present Simple": [
        ("She ___ to work every day.", ["go", "goes", "going"], "goes", "He/She/It ile fiile -s gelir."),
        ("___ you like coffee?", ["Do", "Does", "Did"], "Do", "You ile soru yardımcı fiili Do kullanılır."),
        ("My brother ___ football on Sundays.", ["play", "plays", "playing"], "plays", "My brother = he; bu yüzden plays."),
        ("They ___ English at school.", ["study", "studies", "studying"], "study", "They ile fiilin yalın hali kullanılır."),
        ("He ___ not like fish.", ["do", "does", "did"], "does", "He ile olumsuz yapıda does not kullanılır."),
    ],
    "Past Simple": [
        ("I ___ a movie yesterday.", ["watch", "watched", "watching"], "watched", "Yesterday geçmiş zamanı gösterir."),
        ("She ___ to Ankara last week.", ["go", "went", "gone"], "went", "Go fiilinin Past Simple hali went'tir."),
        ("Did you ___ breakfast?", ["ate", "eat", "eaten"], "eat", "Did'den sonra fiilin yalın hali gelir."),
        ("We ___ at home last night.", ["stay", "stayed", "staying"], "stayed", "Last night geçmiş zamanı gösterir."),
        ("He didn't ___ to school.", ["went", "go", "gone"], "go", "Didn't'den sonra fiilin yalın hali gelir."),
    ],
    "Present Continuous": [
        ("I ___ studying now.", ["am", "is", "are"], "am", "I ile am kullanılır."),
        ("She is ___ TV.", ["watch", "watched", "watching"], "watching", "is + fiil-ing kullanılır."),
        ("They ___ working now.", ["am", "is", "are"], "are", "They ile are kullanılır."),
        ("What are you ___?", ["do", "did", "doing"], "doing", "are + fiil-ing yapısı gerekir."),
        ("He ___ sleeping at the moment.", ["am", "is", "are"], "is", "He ile is kullanılır."),
    ],
    "Future": [
        ("I ___ call you tomorrow.", ["will", "did", "am"], "will", "Tomorrow gelecek zamanı gösterir."),
        ("She is going to ___ dinner.", ["cook", "cooked", "cooking"], "cook", "Going to'dan sonra fiilin yalın hali gelir."),
        ("We ___ travel next month.", ["will", "did", "were"], "will", "Next month gelecek zamanı gösterir."),
        ("They are going to ___ a new car.", ["buy", "bought", "buying"], "buy", "Going to + fiilin yalın hali."),
        ("I think it ___ rain tomorrow.", ["will", "did", "was"], "will", "Tahmin için will kullanılabilir."),
    ],
    "Present Perfect": [
        ("I ___ visited London.", ["have", "has", "did"], "have", "I ile have + V3 kullanılır."),
        ("She has ___ sushi before.", ["eat", "ate", "eaten"], "eaten", "Has'tan sonra V3 gerekir."),
        ("Have you ever ___ abroad?", ["travel", "traveled", "traveling"], "traveled", "Have + V3 kullanılır."),
        ("He ___ never seen snow.", ["have", "has", "did"], "has", "He ile has kullanılır."),
        ("We have ___ this movie.", ["see", "saw", "seen"], "seen", "Have'tan sonra see fiilinin V3 hali seen gelir."),
    ],
    "Questions": [
        ("___ you like music?", ["Do", "Does", "Did"], "Do", "You ile Do kullanılır."),
        ("___ she work here?", ["Do", "Does", "Did"], "Does", "She ile Does kullanılır."),
        ("___ you go yesterday?", ["Do", "Does", "Did"], "Did", "Yesterday geçmiş zaman olduğu için Did."),
        ("Where ___ he live?", ["do", "does", "did"], "does", "He ile does kullanılır."),
        ("What ___ they do every day?", ["do", "does", "did"], "do", "They ile do kullanılır."),
    ],
    "Prepositions": [
        ("I wake up ___ 7 o'clock.", ["in", "on", "at"], "at", "Saatlerde at kullanılır."),
        ("I work ___ Monday.", ["in", "on", "at"], "on", "Günlerde on kullanılır."),
        ("My birthday is ___ July.", ["in", "on", "at"], "in", "Aylarda in kullanılır."),
        ("She is ___ the kitchen.", ["in", "on", "at"], "in", "Bir odanın/alanın içinde in kullanılır."),
        ("The meeting is ___ 3 pm.", ["in", "on", "at"], "at", "Belirli saatlerde at kullanılır."),
    ],
    "Articles": [
        ("I saw ___ dog in the park.", ["a", "an", "the"], "a", "İlk kez bahsedilen tekil sayılabilir isim için a."),
        ("She ate ___ apple.", ["a", "an", "the"], "an", "Sesli harf sesiyle başlayan apple için an."),
        ("___ sun is very bright.", ["A", "An", "The"], "The", "Tek ve belirli varlıklar için the."),
        ("He is ___ doctor.", ["a", "an", "the"], "a", "Mesleklerden önce a/an kullanılır."),
        ("I bought ___ umbrella.", ["a", "an", "the"], "an", "Umbrella sesli harf sesiyle başladığı için an."),
    ],
    "Modal Verbs": [
        ("I can ___ English.", ["speak", "speaks", "speaking"], "speak", "Can'den sonra fiilin yalın hali gelir."),
        ("You should ___ more.", ["study", "studies", "studying"], "study", "Should'dan sonra yalın fiil gelir."),
        ("We must ___ now.", ["go", "goes", "going"], "go", "Must'tan sonra yalın fiil gelir."),
        ("She can ___ very fast.", ["run", "runs", "running"], "run", "Can'den sonra yalın fiil gelir."),
        ("You should ___ careful.", ["be", "are", "being"], "be", "Should'dan sonra be'nin yalın hali gelir."),
    ],
}

def detect_grammar_topic(question, answer=""):
    q = str(question.get("english", question) if isinstance(question, dict) else question).lower()
    if any(x in q for x in ["did you", "yesterday", "last ", "ago", "what did"]): return "Past Simple"
    if any(x in q for x in ["have you ever", "has she", "have you", "have they"]): return "Present Perfect"
    if any(x in q for x in ["going to", "tomorrow", "next week", "next month", "next year"]): return "Future"
    if any(x in q for x in ["now", "at the moment", "right now"]): return "Present Continuous"
    if q.startswith(("do ", "does ", "did ")) or " do you " in q or " does he " in q: return "Questions"
    if any(x in q for x in ["at ", "on ", "in "]) and any(x in q for x in ["o'clock", "monday", "january", "july"]): return "Prepositions"
    return "Present Simple"

def record_grammar_result(topic, correct):
    data = st.session_state.grammar_weakness.setdefault(topic, {"attempts": 0, "wrong": 0})
    data["attempts"] += 1
    if not correct: data["wrong"] += 1

def grammar_weakest_topics(limit=3):
    ranked=[]
    for topic,data in st.session_state.grammar_weakness.items():
        a=data.get("attempts",0); w=data.get("wrong",0)
        if a: ranked.append((w/a, a, topic))
    ranked.sort(key=lambda x:(-x[0],-x[1]))
    return [x[2] for x in ranked[:limit]]

def start_grammar_coach(topic=None):
    if topic is None:
        weak=grammar_weakest_topics(1)
        topic=weak[0] if weak else "Present Simple"
    topic=topic if topic in GRAMMAR_TOPICS else "Present Simple"
    st.session_state.grammar_lesson=topic
    st.session_state.grammar_quiz=list(GRAMMAR_QUIZZES.get(topic, GRAMMAR_QUIZZES["Present Simple"]))
    random.shuffle(st.session_state.grammar_quiz)
    st.session_state.grammar_quiz_index=0
    st.session_state.grammar_quiz_score=0
    st.session_state.grammar_quiz_feedback=None
...
if menu == "🏠 Ana Sayfa":
    ...
    # MODERN DASHBOARD
    # ========================================================
    daily_goal_xp = st.session_state.daily_goal.get("xp", 100)
    today_xp = st.session_state.daily_progress.get("xp", 0)
    daily_ratio = min(1.0, today_xp / max(1, daily_goal_xp))
    current_cefr = kartal_question_level(st.session_state.get("xp", 0))
    next_xp = max(0, level_target - level_xp)

    st.markdown(f"""
    <div class="k-dashboard">
      <div class="k-panel">
        <div class="k-small">BUGÜNÜN HEDEFİ</div>
        <div class="k-big">{today_xp} / {daily_goal_xp} XP</div>
        <p>Günlük hedefe ilerliyorsun. Birkaç pratik daha yap ve seriyi koru.</p>
        <div class="k-bar"><div style="width:{daily_ratio*100:.0f}%"></div></div>
        <div class="k-small">%{daily_ratio*100:.0f} tamamlandı</div>
      </div>
      <div class="k-panel">
        <div class="k-small">AKTİF SEVİYE</div>
        <div class="k-big">{current_cefr}</div>
        <p>K.A.R.T.A.L. soruları performansına göre ayarlıyor.</p>
        <div class="k-small" style="margin-top:12px;">🏆 Seviye {level} • ⭐ {st.session_state.xp} XP</div>
        <div class="k-small">🔥 {st.session_state.streak} günlük çalışma serisi</div>
      </div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### 🗺️ Öğrenme Haritası")
    st.caption("Her seviyede ilerledikçe K.A.R.T.A.L. daha zor ve daha kapsamlı sorulara geçer.")

    level_order = ["A1", "A2", "B1", "B2"]
    level_topics = {
        "A1": ["Tanışma & temel ifadeler", "Günlük hayat", "Temel kelimeler"],
        "A2": ["Geçmiş zaman", "Hobiler & alışkanlıklar", "Seyahat"],
        "B1": ["Fikir belirtme", "Deneyimler", "Gelecek planları"],
        "B2": ["Akıcı konuşma", "Karma grammar", "Soyut konular"],
    }
    active_index = level_order.index(current_cefr) if current_cefr in level_order else 0

    cards = []
    for i, cefr in enumerate(level_order):
        cls = "k-level active" if i == active_index else ("k-level locked" if i > active_index else "k-level")
        status = "AKTİF" if i == active_index else ("TAMAMLANDI" if i < active_index else "KİLİTLİ")
        items = "".join(f"<li>{x}</li>" for x in level_topics[cefr])
        cards.append(f"""
        <div class="{cls}">
        ...
        """)
    st.markdown('<div class="k-map">' + ''.join(cards) + '</div>', unsafe_allow_html=True)

    recommendation = learning_recommendation()
    st.markdown(f"""
    <div class="k-recommend">
      <strong>🎯 K.A.R.T.A.L. önerisi</strong><br>
      <span style="color:#c7d2fe !important;">{recommendation}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.divider()
    st.subheader("🧑‍🏫 K.A.R.T.A.L. Grammar Koçu")
    grammar_weak = grammar_weakest_topics(3)
    if grammar_weak:
        st.write("🔴 Zayıf grammar alanların:")
        for t in grammar_weak:
            d=st.session_state.grammar_weakness[t]
            acc=round(100*(d["attempts"]-d["wrong"])/max(1,d["attempts"]))
            st.write(f"• **{t}** — %{acc} başarı")
    else:
        st.caption("Henüz grammar verisi oluşmadı. Konuşma pratiği yaptıkça K.A.R.T.A.L. zayıf noktalarını bulacak.")

    selected_topic = st.selectbox("Mini ders konusu", list(GRAMMAR_TOPICS.keys()), key="grammar_topic_select")
    if st.button("🧠 Grammar Koçunu Başlat", key="start_grammar_coach"):
        start_grammar_coach(selected_topic)
        st.rerun()

    if st.session_state.grammar_lesson:
        topic=st.session_state.grammar_lesson
        lesson=GRAMMAR_TOPICS[topic]
        st.markdown(f"### 📘 {topic} — {lesson['tr']}")
        st.write(lesson["explanation"])
        st.write("**Örnekler:**")
        for ex in lesson["examples"]: st.write(f"• {ex}")

        quiz=st.session_state.grammar_quiz
        idx=st.session_state.grammar_quiz_index
        if idx < len(quiz):
            q, options, answer, explanation = quiz[idx]
            st.markdown(f"**Soru {idx+1}/5:** {q}")
            choice=st.radio("Cevabın", options, key=f"grammar_choice_{topic}_{idx}", horizontal=True)
            if st.button("Cevabı kontrol et", key=f"grammar_check_{topic}_{idx}"):
                ok=(choice==answer)
                record_grammar_result(topic, ok)
                if ok:
                    st.session_state.grammar_quiz_score += 1
                    st.session_state.grammar_quiz_feedback=(True, f"✅ Doğru! {explanation}")
                else:
                    st.session_state.grammar_quiz_feedback=(False, f"❌ Doğru cevap: **{answer}** — {explanation}")
                st.session_state.grammar_quiz_index += 1
                st.rerun()
        else:
            score=st.session_state.grammar_quiz_score
            st.success(f"🎉 Grammar mini test tamamlandı: **{score}/5** doğru.")
            if score < 3: st.warning("Bu konuyu biraz daha çalışalım. K.A.R.T.A.L. bunu zayıf alanlarına ekledi.")
            elif score < 5: st.info("İyi gidiyorsun. Birkaç tekrar daha ile pekiştirebiliriz.")
            else: st.success("🏆 Mükemmel! Bu grammar konusu çok iyi durumda.")
            if st.button("🔄 Aynı konuyu tekrar çöz", key=f"grammar_restart_{topic}"):
                start_grammar_coach(topic)
                st.rerun()
    ...
    if sentence is None:
        ...
        if audio:
            ...
            if spoken_text:
                ...
                # TTS her zaman ekranda sorulan HEDEF cümleyi söylemeli.
                st.info(
                    f"🔊 K.A.R.T.A.L. doğru cümleyi söylüyor: **{sentence['english']}**"
                )
                ...
                # Sonuç ne olursa olsun ses yalnızca hedef cümleyi okur.
                speak_text(sentence["english"])
            ...
            if spoken:
                ...
                if spoken.strip() == st.session_state.conversation_last_answer.strip():
                    ...
                    try:
                        ...
                        conv_topic = detect_grammar_topic(question)
...
st.markdown("""
<style>
/* Ana sayfa: beyaz zemin sorununu tamamen kaldır */
html, body, .stApp,
[data-testid="stAppViewContainer"],
...
/* Ana içerikteki tüm normal yazılar görünür */
[data-testid="stAppViewContainer"] .main,
[data-testid="stAppViewContainer"] .main * {
    color: #f8fafc !important;
}

/* Streamlit'in markdown alanları */
...
/* Input, selectbox, text area, butonlar */
...
/* Butonları modern koyu kart şeklinde göster */
...
st.markdown("""
<div style="text-align:center;opacity:.45;font-size:.78rem;padding:30px 0 5px;">
🦅 K.A.R.T.A.L. English • Smart Learning System
</div>
""", unsafe_allow_html=True)
